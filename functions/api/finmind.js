...
//   若快取涵蓋查詢區間（容許最新幾天的更新延遲）則直接回傳（省 FinMind 額度）；
...
// 每次都消耗 FinMind API 額度。快取判斷容許最新幾天的更新延遲
// （STATIC_CACHE_TAIL_TOLERANCE_DAYS），避免查詢區間結尾為「今天」時
// 因當日排程尚未跑完而每次都白白 fallback 回即時 FinMind。
//
...
// 快取結尾容許落後查詢結束日的天數上限（例如今天的排程尚未跑完）；
// 超過此天數才視為快取「過舊、不足以覆蓋」，改為即時查詢 FinMind
const STATIC_CACHE_TAIL_TOLERANCE_DAYS = 5;
...
export async function onRequestGet({ request, env }) {
  ...
  if (STATIC_CACHE_CONFIG[dataset]) {
              ...
              // 快取未覆蓋查詢區間、讀取失敗，或缺少日期參數：落到下方即時 FinMind 代理
  }
...
// 只有當快取的日期範圍「涵蓋」請求的 start~end 時才採用（結尾容許
// STATIC_CACHE_TAIL_TOLERANCE_DAYS 天的更新延遲），否則回傳 null，
// 讓呼叫端 fallback 到即時 FinMind 代理。
async function tryStaticCache(origin, dataset, start, end, dataId) {
  ...
  if (cacheMin > start) return null; // 快取涵蓋不到查詢起始日，交給即時查詢

  if (cacheMax < end) {
              const gapDays = daysBetween(cacheMax, end);
              // 落後太多天（例如查詢區間遠早於快取涵蓋範圍、或快取已停止更新）
            // 才視為不足以覆蓋；落後幾天內（例如今天排程尚未跑完）仍視為可用
            if (gapDays > STATIC_CACHE_TAIL_TOLERANCE_DAYS) return null;
  }
